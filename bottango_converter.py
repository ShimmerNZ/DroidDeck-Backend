...
class BottangoConverter:
    ...
    def bottango_pwm_to_maestro_units(self, bottango_value: int, 
        ...
        """
        Convert Bottango scaled integer value to DroidDeck microseconds.
        
        Bottango uses a scaled integer format (0-8192) for animation values.
        We need to convert this to PWM microseconds based on the servo's min/max range.
        
        Formula: PWM_us = (scaled_int / 8192) * (max - min) + min
        """
        # Bottango's scaled int max (from driver config)
        SCALED_INT_MAX = 8192.0
        
        if servo_setup:
            # Servo setup min/max are in microseconds
            min_us = servo_setup.min_pwm
            max_us = servo_setup.max_pwm
            
            # Convert scaled int to PWM microseconds
            pwm_range = max_us - min_us
            normalized = bottango_value / SCALED_INT_MAX
            pwm_us = normalized * pwm_range + min_us
            
            return int(round(pwm_us))
        else:
            # No setup info - assume scaled int represents 992-2000µs range
            min_us = 992.0
            max_us = 2000.0
            pwm_range = max_us - min_us
            normalized = bottango_value / SCALED_INT_MAX
        ...
        for json_file in json_files:
            ...
            if output_paths:
                ...
                if self.delete_after_conversion:
                    try:
                        json_file.unlink()
                        logger.info(f"Deleted source: {json_file.name}")
                    except Exception as e:
